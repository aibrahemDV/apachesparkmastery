...
sales_data = spark.read.format("csv").option("Header", "True").schema(sales_schema).load(
    "D:\\DV\\Courses\\Apache-Spark\\Sources\\intelliji-ws\\SparkCourse-v2\\src\\main\\resources"
    "\\sales_data.csv")
...
employees_data = spark.read.format("csv").option("Header", "True").option("InferSchema", "true") \
    .load("D:\\\\DV\\\\Courses\\\\Apache-Spark\\\\Sources\\\\intelliji-ws\\\\SparkCourse-v2\\\\src\\\\main"
          "\\\\resources\\\\HR_Employees.csv")
...
# employees_data.select("EmployeeID","LastName","FirstName","ReportsTo").show() employees_data.alias("emp").join(
# employees_data.alias('manager'), expr( "emp.EmployeeID=manager.ReportsTo"), "left_outer").select(expr(
# "emp.EmployeeID"), expr("emp.FirstName||' '||emp.LastName as Employee_Name"), expr("manager.FirstName||' '||
# manager.LastName as Manager_Name")).show()
